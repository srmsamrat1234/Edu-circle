...
const ChatInterface = ({ isOpen, onClose }) => {
  ...
  // ========== FIXED: OPEN/DOWNLOAD FILE - Remove /api prefix for static files ==========
  const handleOpenFile = (fileUrl, fileName, fileType) => {
    ...
    // Remove /api suffix if present (for static file access)
    if (baseURL.endsWith('/api')) {
      baseURL = baseURL.replace(/\/api$/, '');
    }
    
    // Also remove /api prefix from fileUrl if present
            ...
            <>
              ...
              <div className="bg-blue-600 text-white p-4 flex items-center gap-3 shadow-sm">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-100 capitalize">{selectedUser.role}</p>
                </div>
              </div>
                      ...
                      <div className={`max-w-[70%] p-3 rounded-xl ${msg.sender._id === user._id ? 'bg-blue-600 text-white' : 'bg-white text-gray-900 border border-gray-200'}`}>
                        ...
                        {msg.fileType && msg.fileUrl ? (
                          <div className="space-y-2">
                            ...
                            {msg.fileType === 'image' && (
                              <img 
                                src={`${(API.defaults.baseURL || 'http://localhost:5000').replace(/\/api$/, '')}${msg.fileUrl.replace(/^\/api/, '')}`} 
                                alt="Shared image" 
